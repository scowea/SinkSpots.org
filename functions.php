...
function db_connect()
{
	...
	$con = mysql_connect('sinkspots.db.10211257.hostedresource.com', 'sinkspots', 'slink2Sink!'); 
	...
	mysql_select_db("sinkspots", $con); //database
...
{
	// open the form
   	echo "<form action=$current_page_name?spot_id=$spot_id&action=jump_to_spot name=jump_to_spot   method=post> ";
	
	//select all the mystery spots...
	$result = mysql_query("SELECT * FROM `mystery_spots` ORDER BY `mystery_spot_name`" );

	// open tag
	echo '<select name=spot_id onchange="this.form.submit();" class="mystery_jump_to_spot_dropdown">';
...
function build_tab_navigation_list($currently_selected_id, $currently_selected_tab)
{
	...
	<div id=navigation> 
     	<ul>";
			...
			// EXPLORE
			if ($currently_selected_tab == "Map") 
        		echo "<li><a href=explore.php?spot_id=" . $currently_selected_id . " class=selected>Map</a></li>";
			else
				echo "<li><a href=explore.php?spot_id=" . $currently_selected_id . ">Map</a></li>";
...
<?php 	
	...
	echo " 
      <br><br>
      <h1> Sink<font color=#000000>Spots</font><font size=-2>.org</font>&nbsp;&nbsp;&nbsp;<font size=-1>[<a href=settings_maintenance.php?spot_id=" . $spot_id . ">settings</a>] [<a href=about.php?spot_id=" . $spot_id . ">about</a>]  </font> &nbsp;&nbsp;<font size=+1>[<a href=whats_in.php?spot_id=" . $spot_id . "><font color=#009900>WHATS IN RIGHT NOW!</font></a>]</font><font size=-1> Boat Gallery! ->  [<a href='./boats.php'>For Sale/For Show</a>]</font> </h1>
	...
	while ($record = mysql_fetch_array($result, MYSQL_ASSOC))
	{
		...
		//		$action_taken = "log";
			...
			else if ($table_affected == "links")
			{
				...
				echo "$user_name - $action_taken link on $time_stamp ~ <a href='".$link."' target=_blank>".$label."</a>";
				echo "<br>";
			}
			...
			else if ($table_affected == "users")
			{
				...
				{
						// show it...
					echo "<font color=#00FF00>" . $record['new_value'] . " - $action_taken user account on $time_stamp </font>";
					echo "<br>";				
				}
			}			
			else
...
function build_list_of_spots_that_are_in()
{
	...
	while ($spot_row = mysql_fetch_array($result, MYSQL_ASSOC)) 
	{
		...
		// print the spot to the screen as a link with its id....no brackets..
		echo "<td><a href=http://sinkspots.com/index.php?spot_id=" . $spot_row['spot_id'] . "><font color=" . $color . ">" . $spot_row['mystery_spot_name'] . "</font></a></td>";
