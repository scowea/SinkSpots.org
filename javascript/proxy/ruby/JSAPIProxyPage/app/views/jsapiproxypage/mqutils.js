...
function mqGetBrowserInfo()
{
   ...
   if((appVersion.toLowerCase().indexOf('macintosh')+1)?true:false)
   {
      if(!browser.isNS && !browser.isIE)
...
function mqReplaceElementNode(xmlDoc,nodeDoc,xpath) {
   ...
   var oldnode = xmlDoc.getElementsByTagName(xpath)[0];
   if (oldnode){
...
function mqLogTime(str)
{
   if(mqGetElementById("mqTimeLogs"))
   {
     ...
     if( _mqLogStartTime == null) {
      logtext.value = "Time\t Difference\t Message\n";
